        ...
        <div className="container mx-auto max-w-md px-4">
          <div className="border border-zinc-200 bg-white p-8 shadow-sm">
            ...
            <form onSubmit={handleAuth} className="space-y-4">
              {!isLoginView && <Input value={name} onChange={(e) => setName(e.target.value)} className="h-12 rounded-none border-zinc-300 text-[13px] focus-visible:ring-[#7C3AED]" required={!isLoginView} placeholder="Full Name" />}
              <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="h-12 rounded-none border-zinc-300 text-[13px] focus-visible:ring-[#7C3AED]" required placeholder="Email Address" />
              <Input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="h-12 rounded-none border-zinc-300 text-[13px] focus-visible:ring-[#7C3AED]" required minLength={6} placeholder="Password" />
              <Button type="submit" disabled={isAuthLoading} className="mt-4 h-12 w-full rounded-none bg-[#222222] text-[12px] font-bold uppercase tracking-[0.1em] text-white hover:bg-[#7C3AED]">{isAuthLoading ? "Processing..." : isLoginView ? "Log In" : "Create Account"}</Button>
            </form>
            <div className="mt-6 text-center text-[13px] text-zinc-500">{isLoginView ? "Don't have an account? " : "Already have an account? "}<button onClick={() => setIsLoginView(!isLoginView)} className="font-bold text-zinc-900 hover:text-[#7C3AED]">{isLoginView ? "Register Now" : "Log In"}</button></div>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="bg-[#F4F4F4] pb-24">
      ...
      <div className="container mx-auto px-4 py-12">
        {isLoadingOrders ? <div className="h-32 animate-pulse border border-zinc-200 bg-white" /> : orders.length === 0 ? <div className="border border-zinc-200 bg-white p-12 text-center text-zinc-500">No orders found. <Link to="/shop" className="font-bold text-[#7C3AED]">Start Shopping.</Link></div> : (
            ...
            {orders.map((order) => (
              <div key={order.id} className="border border-zinc-200 bg-white p-6 transition-shadow hover:shadow-sm">
                ...
                <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                  ...
                  <Button asChild variant="outline" className="h-10 rounded-none border-zinc-300 text-[11px] font-bold uppercase tracking-widest text-zinc-900 hover:bg-[#7C3AED] hover:text-white"><Link to={`/track/${order.id}`}>Track / View Details</Link></Button>
                </div>
              </div>
            ))}
          </div>
